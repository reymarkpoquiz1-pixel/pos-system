...
class ProductDetailsPanel extends StatelessWidget {
                  ...
                  builder: (context, constraints) {
                    ...
                    if (isMobile) {
                      ...
                      return Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _buildGallerySidebar(fullWidth: true),
                          const SizedBox(height: 16),
                    ...
                    return Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        SizedBox(width: 200, child: _buildGallerySidebar()),
                        const SizedBox(width: 16),
                        Expanded(child: _buildMainFormFields()),
                      ],
                    );
                  },
                ),
  ...
  Widget _buildGallerySidebar({bool fullWidth = false}) {
    return Column(
      children: [
        Container(
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: activeImageIndex < serverImages.length
                      ? DecorationImage(
                          image: NetworkImage(serverImages[activeImageIndex].startsWith('http') ? serverImages[activeImageIndex] : '$baseUrl/${serverImages[activeImageIndex]}'),
                          fit: BoxFit.contain,
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              image: DecorationImage(image: NetworkImage(entry.value.startsWith('http') ? entry.value : '$baseUrl/${entry.value}'), fit: BoxFit.cover),
                            ),
                          ),
                          if (isEditing)
                            Positioned(
                              ...
                              child: GestureDetector(
                                ...
                                child: Container(padding: const EdgeInsets.all(1), decoration: const BoxDecoration(color: Colors.red, shape: BoxShape.circle), child: const Icon(Icons.close, size: 10, color: Colors.white)),
                              ),
                            ),
                        ],
                      ),
                    ...
                    Material(
                      ...
                      child: InkWell(
                        ...
                        child: Container(
                          width: 40,
                          height: 40,
                          decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFFD68A96), width: 1.5)),
              ...
              Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFFD68A96), borderRadius: BorderRadius.circular(10)),
                child: const Text('Media Gallery', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, color: Colors.white, fontWeight: FontWeight.w600)),
              ),
            ],
          ),
        ),
        ...
        Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFFFCE1E5), borderRadius: BorderRadius.circular(20)),
          child: Column(
            children: [
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('Product Tags', style: TextStyle(fontSize: 12, color: Color(0xFF1C1B1F), fontWeight: FontWeight.w600)),
                  if (isEditing)
                    Material(
                      ...
                      child: InkWell(
                        ...
                        borderRadius: BorderRadius.circular(8),
                        child: const Padding(padding: EdgeInsets.symmetric(horizontal: 10, vertical: 4), child: Text('Add Tag', style: TextStyle(fontSize: 11, color: Colors.white, fontWeight: FontWeight.bold))),
                      ),
                    ),
                ],
              ),
              ...
              Wrap(
                spacing: 4,
                runSpacing: 4,
                children: productTags.map((tag) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: const Color(0xFFD68A96), borderRadius: BorderRadius.circular(8)), child: Row(mainAxisSize: MainAxisSize.min, children: [Text(tag, style: const TextStyle(fontSize: 11, color: Colors.white, fontWeight: FontWeight.w600)), const SizedBox(width: 4), if (isEditing) GestureDetector(onTap: () => onRemoveTag(tag), child: const Icon(Icons.close, size: 10, color: Colors.white))]))).toList(),
              ),
              const SizedBox(height: 12),
      ...
      children: [
            ...
            children: [
              Row(
                children: [
                  ...
                  Expanded(flex: 2, child: ProductInputField(label: 'Barcode', controller: barcodeController, hint: 'Scan...', readOnly: !isEditing, suffix: isEditing ? IconButton(icon: const Icon(Icons.auto_fix_high, size: 18, color: Color(0xFFD68A96)), onPressed: onGenerateBarcode, padding: EdgeInsets.zero, constraints: const BoxConstraints()) : null)),
                ],
              ),
              const SizedBox(height: 8),
              CategorySelector(isEditing: isEditing, selectedCategoryName: selectedCategoryName, selectedCategoryId: selectedCategoryId, categories: categories, onCategorySelected: onCategorySelected),
            ],
          ),
        ),
        const SizedBox(height: 12),
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 12),
                  Expanded(child: ProductInputField(label: 'Selling Price (P)', controller: sellingPriceController, hint: '0.00', readOnly: !isEditing)),
                ],
              ),
              const SizedBox(height: 8),
              Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ProductInputField(label: 'Total Stock', controller: stockController, hint: '0', readOnly: !isEditing || variants.isNotEmpty), if (variants.isNotEmpty) const Padding(padding: EdgeInsets.only(top: 4, left: 4), child: Text('Sum of all variants', style: TextStyle(fontSize: 10, color: Color(0xFFD68A96), fontStyle: FontStyle.italic)))])),
        ...
        Container(
          ...
          child: SingleChildScrollView(
            ...
            child: SizedBox(
              width: 450, // Minimum width to prevent variant table squishing
              child: ProductVariantsTable(variants: variants, isEditing: isEditing, onAddVariant: onAddVariant, onRemoveVariant: onRemoveVariant, onEditVariant: onEditVariant),
            ),
          ),
        ),
        const SizedBox(height: 12),
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                height: 80,
                decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), border: Border.all(color: const Color(0xFFEFAFB8), width: 1)),
                child: TextField(controller: descriptionController, maxLines: 3, readOnly: !isEditing, enabled: isEditing, style: const TextStyle(fontSize: 13, color: Colors.black), decoration: const InputDecoration(hintText: 'Enter product description...', hintStyle: TextStyle(color: Colors.black38, fontSize: 13), contentPadding: EdgeInsets.all(10), border: InputBorder.none)),
              ),
            ],
          ),
        ),
  ...
  Widget _buildSaveButton() {
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFD68A96), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0),
        child: isSaving ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Text('Save Product', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      ),
    );
  }
